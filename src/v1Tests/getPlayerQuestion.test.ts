...
describe('Test for GET /v1/player/quiz/:quizId/question/:questionId', () => {
  // successful cases
  test('200: successfully fetch player question', () => {
    const response = getPlayerQuestionRequest(token, quiz.quizId, questionId);
  ...
  test('400: invalid token provided', () => {
    const response = getPlayerQuestionRequest('invalid-token', quiz.quizId, questionId);
    expect(response.statusCode).toBe(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual(ERROR);
  });

  test('400: invalid quizId provided', () => {
    const response = getPlayerQuestionRequest(token, quiz.quizId + 1, questionId);
    expect(response.statusCode).toBe(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual(ERROR);
  });

  test('400: invalid questionId provided', () => {
    const response = getPlayerQuestionRequest(token, quiz.quizId, questionId + 1);
    expect(response.statusCode).toBe(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual(ERROR);
  });
});