...
const requestAdminQuestionDuplicate = (quizId: number, quizQuestionId: number, body: { token: string }) => {
  return request ('POST', `${SERVER_URL}/v1/admin/quiz/${quizId}/question/${quizQuestionId}/duplicate`, {
    json: {
      token: body.token,
    }
  });
...
describe('HTTP tests for /v1/admin/quiz/{quizId}/name', () => {
  ...
  beforeEach(() => {
    ...
    question = request('POST', `${SERVER_URL}/v1/admin/quiz/${quizId}/question`, {
      json: { 
        token: token, 
        questionBody: {
  ...
  test('200: successful case', () => {
    const response = requestAdminQuestionDuplicate(quiz.quizId, question.questionId, { token: token });

    expect(response.statusCode).toBe(200);
    expect(JSON.parse(response.body.toString())).toStrictEqual({ duplicateQuestionId: expect.any(Number) });
  });

  test('400: QuestionId does not refer to a valid question within the quiz', () => {
    const response = requestAdminQuestionDuplicate(quiz.quizId, question.questionId + 1, { token: token });
  ...
  test('401: Token is invalid', () => {
    const response = requestAdminQuestionDuplicate(quiz.quizId, question.questionId, { token: 'invalidToken' });
  ...
  test('403: Valid token but the quiz does not exist', () => {
    const response = requestAdminQuestionDuplicate(quiz.quizId + 1, question.questionId, { token: token });
