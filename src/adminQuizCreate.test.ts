...
describe('testing for adminQuizCreate', () => {
  test('successful creation of quiz returns valid quiz id', () => {
    ...
    expect(adminQuizCreate(authregister.authUserId, name, description))
  ...
  test('nonexistent user id returns error', () => {
    ...
    const name = 'name';
    const description = 'description';
    expect(adminQuizCreate(authUserId, name, description))
  ...
  test('description longer than 100 characters returns error', () => {
    ...
    const authregister = adminAuthRegister('aero@mail.com', 'Aeropass1', 'Jason', 'Chandra');
    const name = 'name';
    const description = 'This description is longer than' +
                        '100 characters and should return error yap yap yap yap yap yap' +
                        'yap yap yap yap';
    expect(adminQuizCreate(authregister.authUserId, name, description))
      .toStrictEqual({ error: expect.any(String) });
  });
  ...
  test.each([
    ...
    return expect(adminQuizCreate(authUserId, name, description))
      .toStrictEqual({ error: expect.any(String) });
  });
});
