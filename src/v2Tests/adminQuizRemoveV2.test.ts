...
describe('DELETE /v1/admin/quiz/:quizId/', () => {
  ...
  test('returns error when trying to delete quiz with invalid token', () => {
    const userToken = adminAuthRegisterWrapper('jake.renzella@gmail.com', 'password123',
      'Jake', 'Renzella');

    const quizId = adminQuizCreate(userToken, 'Basic quiz', 'Just a normal quiz');
  ...
  test('returns error when trying to delete a quiz that user does not own', () => {
    const userToken1 = adminAuthRegisterWrapper('jake.renzella@gmail.com', 'password123',
      'Jake', 'Renzella');

    const quizId = adminQuizCreate(userToken1, 'Basic quiz', 'Just a normal quiz');

    const userToken2 = adminAuthRegisterWrapper('hayden.smith@gmail.com', 'password123',
      'Hayden', 'Smith');
  ...
  test('correct return type for deleting a quiz', () => {
    ...
    const resultRes = request('DELETE', SERVER_URL + `/v1/admin/quiz/${quizId}/`, {
      qs: { token: userToken },
      timeout: TIMEOUT_MS
    });
  ...
  test('correct status code when deleting a quiz', () => {
    ...
    const result = request('DELETE', SERVER_URL + `/v1/admin/quiz/${quizId}/`, {
      qs: { token: userToken },
      timeout: TIMEOUT_MS
    });
